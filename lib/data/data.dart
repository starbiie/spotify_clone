const yourLibrary = [
  'Made For You',
  'Recently Played',
  'Liked Songs',
  'Albums',
  'Artists',
  'Podcasts',
];

const playlists = [
  'Today\'s Top Hits',
  'Discover Weekly',
  'Release Radar',
  'Chill',
  'Background',
  'lofi hip hop music - beats to relax/study to',
  'Vibes Right Now',
  'Time Capsule',
  'On Repeat',
  'Summer Rewind',
  'Dank Doggo Tunes',
  'Sleepy Doge',
];

class Podcastsfit {
  final String id;
  final String title;
  final String date;
  final String duration;
  final String imageUrl;

  const Podcastsfit({
    required this.id,
    required this.title,
    required this.date,
    required this.duration,
    required this.imageUrl,
  });
}

const podcast = [
  Podcastsfit(
    id: '0',
    title: 'All Out 2010s',
    date: 'jul 28',
    duration: '3:15',
    imageUrl: 'assets/allout.png',
  ),
  Podcastsfit(
    id: '1',
    title: 'Healthy Distraction',
    date: 'Apr 2014',
    duration: '2:18',
    imageUrl: 'assets/podcast1.png',
  ),
  Podcastsfit(
    id: '2',
    title: 'Far Away',
    date: 'May 1',
    duration: '2:39',
    imageUrl: 'assets/pd2.png',
  ),
  Podcastsfit(
    id: '3',
    title: 'Call You Soon',
    date: 'Nov 2022',
    duration: '2:35',
    imageUrl: 'assets/pd3.png',
  ),
  Podcastsfit(
    id: '4',
    title: 'Healthy Distraction',
    date: 'Apr 2014',
    duration: '2:18',
    imageUrl: 'assets/pd4.png',
  ),
  Podcastsfit(
    id: '5',
    title: 'Far Away',
    date: 'May 1',
    duration: '2:39',
    imageUrl: 'assets/pd5.png',
  ),
  Podcastsfit(
    id: '6',
    title: 'Call You Soon',
    date: 'Nov 2022',
    duration: '2:35',
    imageUrl: 'assets/deepfocs.png',
  ),
  Podcastsfit(
    id: '7',
    title: 'Healthy Distraction',
    date: 'Apr 2014',
    duration: '2:18',
    imageUrl: 'assets/instrumental.png',
  ),
  Podcastsfit(
    id: '8',
    title: 'Far Away',
    date: 'May 1',
    duration: '2:39',
    imageUrl: 'assets/chillhits.png',
  ),
  Podcastsfit(
    id: '9',
    title: 'Call You Soon',
    date: 'Nov 2022',
    duration: '2:35',
    imageUrl: 'assets/deepfocs.png',
  ),
  Podcastsfit(
    id: '10',
    title: 'All Out 2010s',
    date: 'jul 28',
    duration: '3:15',
    imageUrl: 'assets/allout.png',
  ),
  Podcastsfit(
    id: '11',
    title: 'Healthy Distraction',
    date: 'Apr 2014',
    duration: '2:18',
    imageUrl: 'assets/instrumental.png',
  ),
];

class Song {
  final String id;
  final String title;
  final String artist;
  final String album;
  final String duration;
  final String imageUrl;

  const Song({
    required this.id,
    required this.title,
    required this.artist,
    required this.album,
    required this.duration,
    required this.imageUrl,
  });
}

const lofihiphopMusic = [
  Song(
    id: '0',
    title: 'All Out 2010s',
    artist: 'WYS',
    album: '1 Am. Study Session',
    duration: '3:15',
    imageUrl: 'assets/myPicx.jpg',
  ),
  Song(
    id: '1',
    title: 'Healthy Distraction',
    artist: 'less.people',
    album: 'One Day It\'s Over',
    duration: '2:18',
    imageUrl: 'assets/instrumental.png',
  ),
  Song(
    id: '2',
    title: 'Far Away',
    artist: 'Mila Coolness',
    album: 'Silent River',
    duration: '2:39',
    imageUrl: 'assets/chillhits.png',
  ),
  Song(
    id: '3',
    title: 'Call You Soon',
    artist: 'Louk, Glimlip',
    album: 'Can We Talk',
    duration: '2:35',
    imageUrl: 'assets/deepfocs.png',
  ),
  Song(
    id: '4',
    title: 'Winter Sun',
    artist: 'Bcalm, Banks',
    album: 'Feelings',
    duration: '2:15',
    imageUrl: 'assets/instrumental.png',
  ),
  Song(
    id: '5',
    title: 'Hope',
    artist: 'No Spirit',
    album: 'Memories We Made',
    duration: '1:57',
    imageUrl: 'assets/jazzvibs.png',
  ),
  Song(
    id: '6',
    title: 'A Better Place',
    artist: 'Project AER, cxit.',
    album: 'Growth Patterns',
    duration: '2:00',
    imageUrl: 'assets/lofigirl.jpg',
  ),
  Song(
    id: '7',
    title: 'Misty Dawn',
    artist: 'BluntOne',
    album: 'Autumn in Budapest',
    duration: '2:34',
    imageUrl: 'assets/rockclasic.png',
  ),
  Song(
    id: '8',
    title: 'Hourglass',
    artist: 'Thaehan',
    album: 'Hourglass',
    duration: '1:43',
    imageUrl: 'assets/lofigirl.jpg',
  ),
  Song(
    id: '9',
    title: 'After Sunset',
    artist: 'Project AER, WYS',
    album: '3 Am. Study Session',
    duration: '2:41',
    imageUrl: 'assets/lofigirl.jpg',
  ),
  Song(
    id: '10',
    title: 'Child',
    artist: 'Ambulo',
    album: 'Polar',
    duration: '2:12',
    imageUrl: 'assets/lofigirl.jpg',
  ),
  Song(
    id: '11',
    title: 'Arizona Zero',
    artist: 'WYS, Sweet Medicine',
    album: 'Evermore',
    duration: '2:31',
    imageUrl: 'assets/lofigirl.jpg',
  ),
];

class Playlist {
  final String id;
  final String name;
  final String imageURL;
  final String description;
  final String creator;
  final String duration;
  final String followers;
  final List<Song> songs;
  final List<Podcastsfit> podcastfits;

  const Playlist({
    required this.id,
    required this.name,
    required this.imageURL,
    required this.description,
    required this.creator,
    required this.duration,
    required this.followers,
    required this.songs,
    required this.podcastfits,
  });
}

const lofihiphopPlaylist = Playlist(
  id: '5-playlist',
  name: 'lofi hip hop music - beats to relax/study to',
  imageURL: 'assets/allout.png',
  description:
  'A daily selection of chill beats - perfect to help you relax & study 📚',
  creator: 'Lofi Girl',
  duration: '28 min',
  followers: '5,351,685',
  songs: lofihiphopMusic, podcastfits: podcast,
);